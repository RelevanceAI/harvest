...
```python
@app.function(
    scaledown_window=300,  # 5 minutes idle before shutdown
    min_containers=0,      # Don't keep warm (save costs)
    memory=6144,           # 6GB (Claude + Node + Python processes)
    timeout=3600,          # 1 hour max session duration
)
```
...
**Decision:** Terminate at 5 minutes idle, save conversation history to SQLite, replay on next start. Saves ~80% compute cost with acceptable <3s cold start penalty.
...
2. **Session duration limit:** 1 hour max per sandbox
3. **Idle timeout:** 5 minutes (restarts PTY, clears memory)
4. **Manual restart trigger:** If memory >5GB, force restart before next prompt
...
class HarvestSandbox:
    ...
    async def start(self, timeout_secs: int = 3600):
        """Start sandbox with PTY-based Claude CLI"""
...
|------|--------|------------|------------|
| **Claude memory leaks** | High | High | 6GB container limit, 1-hour max session, 5-min idle timeout |
...
3. **Long-running tasks:** Should we support tasks >1 hour by checkpointing and resuming?
