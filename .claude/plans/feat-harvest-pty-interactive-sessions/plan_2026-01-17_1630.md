...
@app.function(
    ...
    timeout=86400,         # 24 hour session timeout (supports overnight tasks)
)
```

**Timeout types:**
1. **Idle timeout (5 min)**: When user sends NO messages, container shuts down to save costs. Conversation history saved to SQLite.
2. **Session timeout (24 hours)**: Maximum duration for a single task. Supports overnight work like "refactor entire auth system".
...
**Decision:** Terminate at 5 minutes idle (no messages), but allow up to 24 hours for active long-running tasks. Saves ~80% compute cost with acceptable <3s cold start penalty.
...
2. **Session timeout:** 24 hours max (supports overnight tasks like "refactor auth system")
...
class HarvestSandbox:
    ...
    async def start(self, timeout_secs: int = 86400):
        """Start sandbox with PTY-based Claude CLI (24-hour session timeout)"""
...
@app.function(
    ...
    timeout=86400,            # 24 hour session timeout (overnight tasks)
)
```
...
|------|--------|------------|------------|
| **Claude memory leaks** | High | High | 6GB container limit, 24-hour session timeout, 5-min idle timeout, memory monitoring |
...
3. **Extended sessions:** Should we support tasks >24 hours with checkpointing? Current 24-hour limit supports overnight work (core use case).
