...
## Estimation Guidelines (AI-Driven Development)

When estimating tasks, assume **AI is building** and humans are driving/reviewing. This dramatically changes estimates.

### AI vs Human Estimation

| Task Type | Human Estimate | AI Estimate | Ratio |
|-----------|----------------|-------------|-------|
| Boilerplate/scaffolding | 4-8h | 30-60 min | ~8x faster |
| CRUD/wrapper code | 4-6h | 30-60 min | ~6x faster |
| Core logic (new feature) | 8-16h | 2-4h | ~4x faster |
| Integration/glue code | 4-8h | 1-2h | ~4x faster |
| Test writing | 8-12h | 1-2h | ~6x faster |
| Debugging/iteration | Variable | Variable | ~Same (external dependencies) |

### Why AI is Faster

- **No context switching**: AI holds entire codebase in memory
- **No typing fatigue**: Generates code instantly
- **Pattern recognition**: Recognizes and applies patterns without lookup
- **Parallel exploration**: Can consider multiple approaches simultaneously

### What AI Can't Speed Up

- **External API calls**: Modal builds, CI pipelines, deploy times
- **Waiting for feedback**: Human review cycles
- **Unexpected behavior**: Debugging third-party quirks
- **Ambiguous requirements**: Clarification loops with humans
...
```
AI Estimate = (Human Estimate / 4 to 8) + External Wait Time
```

**Example**:
- Human estimate: 40 hours of coding + 8 hours of Modal builds
- AI estimate: (40 / 6) + 8 = ~15 hours total

### Calendar Time vs Work Time

- **Work time**: Actual AI execution time (usually 1-2 sessions for a block)
- **Calendar time**: Includes review cycles, feedback, and external waits
- **Rule of thumb**: Calendar time ≈ Work time × 2-3 (for review/iteration)

### When Estimating in Plans

Always provide **AI estimates** as the primary estimate. Add notes for:
- External wait times (builds, deploys, API responses)
- Human review checkpoints
- Known unknowns that might slow things down

**Example task estimate**:
```
Task 1.1.4: Sandbox Wrapper
- AI build time: 2-4 hours
- External waits: ~30 min (Modal sandbox testing)
- Review checkpoint: Human approval before proceeding
